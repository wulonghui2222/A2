...
### 方案 A：Fork & Replace（bolt.diy 成为主体）
...
## 4. 决定一切的硬约束：WebContainers

无论哪种方案，只要你要"bolt 式体验"（跑真 Node、真 npm、真 dev server），就绕不开：
...
### 一个折中变量：要不要 WebContainers？

| | 要 WebContainers | 不要 WebContainers |
|---|---|---|
| **产出物** | 真 Node 全栈应用（bolt 全能力） | 仍是静态产物，但升级为多文件 HTML/JS/CSS + Vite 构建 or 沙箱 iframe |
| **代价** | COOP/COEP、子域、移动端降级、画廊变重 | 拿到 bolt 的 artifact 流、diff/revert、编辑器体验，预览保持现有沙箱模式 |
...
- **NFR-03**：若保留服务端代理（方案 B/C），现有约束满足；方案 A 若沿用 bolt 的客户端 key 模式则违反，需要改 bolt 的 provider 层走你的 `/api` 代理
...
1. **目标产出物**：你真正想要的是"能生成全栈 Node 应用"（必须 WebContainers），还是"生成过程的体验升级"（流式文件、编辑器、diff、终端可视化）？这决定方案。
...
5. **工作量预期**：方案 A 最快见效（fork 能跑）但长期嫁接成本高；方案 C 最慢但最干净。你的时间预算量级是？

---

## 7. 直觉倾向

> 如果 A2 的画廊/分享是核心资产，**方案 C 的"偷魂不换命"变体**（artifact 流 + diff 迭代 + 多文件产物，但暂不引入 WebContainers，预览留在现有沙箱体系）是风险收益比最好的一档；等真需要跑 Node 后端时再把 WebContainers 作为二期加进来。
>
> 但如果你要的就是"demo 时能生成真全栈应用"的震撼感，那只能选 A/B 拥抱 WebContainers。
