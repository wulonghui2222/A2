...
直到全部完成，实时呈现进度，任一步骤失败立即暂停并交还用户处置；用户也可随时
手动暂停并在之后恢复推进。
...
N, pausing on failure, wrapping up). The panel SHALL update as steps transition. The
panel SHALL be collapsible: when collapsed, the step list is hidden while the phase
line, a finished/total step count, and the action buttons remain visible. UI copy
SHALL be in Chinese.
...
- **WHEN** the user collapses the progress panel
- **THEN** the step list is hidden while the phase line, a finished/total count, and
  the action buttons remain visible
- **AND** expanding restores the full step list
...
the user. The failure-paused state SHALL present exactly three user-only actions: retry
the failed step, skip it and continue with the next step, or terminate orchestration.
The system SHALL NOT retry automatically. Already-written files SHALL remain available
in the workspace in every outcome.
...
- **WHEN** the user chooses retry on a failure-paused step
- **THEN** the orchestrator starts a fresh round for the same step

#### Scenario: Skip advances past the failed step

- **WHEN** the user chooses skip on a failure-paused step
...
including any skipped steps. The wrap-up SHALL be produced without any model call and
SHALL be persisted as an assistant message carrying a TL role marker through the
existing message persistence channel. After wrap-up the conversation SHALL return to
the normal iteration state.
...
### Requirement: TL-07 User Pause and Resume

**Priority**: P1

While orchestration is actively driving a step round (streaming or settling), the user
SHALL be able to pause orchestration at any time; the paused state SHALL present a
resume action plus terminate. Resuming SHALL continue the same orchestration: if the
paused round had already finished streaming, its settle watch SHALL be re-armed so the
step still completes and advancement continues; if the round was still live, advancement
SHALL proceed from the live stream. A user pause (no failure) SHALL NOT offer the
failure-only retry/skip actions. Pause and resume SHALL be race-free with a round's
completion signal: a round finishing in the gap between the user action and the next UI
render SHALL NOT stall advancement.

#### Scenario: Pause then resume continues advancement

- **GIVEN** orchestration is driving a step round
- **WHEN** the user pauses and later resumes
- **THEN** the orchestration continues driving the remaining steps to completion
- **AND** the wrap-up message is emitted at the end

#### Scenario: Round finishing during pause is not lost

- **GIVEN** the user pauses while a step round is still streaming
- **WHEN** the round's stream finishes while orchestration is paused
- **THEN** the round is recorded and its settle watch is re-armed on resume
- **AND** the step completes and advancement continues
