      ...
      >
        ...
        <div ref={scrollRef} className="flex flex-col lg:flex-row overflow-y-auto w-full h-full">
          <div className={classNames(styles.Chat, 'flex flex-col flex-grow lg:min-w-[var(--chat-min-width)] h-full')}>
            {!chatStarted && (
              <div id="intro" className="mt-[16vh] max-w-chat mx-auto text-center px-4 lg:px-0">
                {/* A2: size tuned so the Chinese heading fits --chat-max-width (37rem)
                    on a single line; nowrap guards against mid-range viewports. */}
                <h1 className="text-2xl lg:text-5xl font-bold text-bolt-elements-textPrimary mb-4 animate-fade-in whitespace-nowrap">
                  用AI构建你的下一个项目
                </h1>
              </div>
            )}
                ...
                >
                    ...
                    <ClientOnly>
                      {() => (
                        <div className={isModelSettingsCollapsed ? 'hidden' : ''}>
                          ...
                          />
                          {/* A2 (design D6 / LG-04, task 6.1): the per-provider API key
                              manager is part of BYOK and stays hidden behind the switch. */}
                          {A2_ENABLE_BYOK && (providerList || []).length > 0 && provider && (
                            <APIKeyManager
                              provider={provider}
                  ...
                  >
                    <textarea
                      ...
                      placeholder="How can A2 help you today?"
                      translate="no"
                    />
                    {/* A2: the bottom icon bar (upload / enhance prompt / speech /
            ...
            {/* A2: everything below the prompt box (import buttons, git clone, example
                prompts, starter templates) is removed from the home page. */}
          </div>
